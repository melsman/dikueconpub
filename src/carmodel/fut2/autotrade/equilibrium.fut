...
module equilibrium (R:real) (trm:trmodel with t = R.t) = {
    ...
    def ded_dprice_tau_full_ad [n][c][Ax][ns][nd] (mp: trm.mp[n][c][Ax][ns][nd]) (tau: i64) (ev:[ns]t)
        (v:[ns][nd]t) (p:trm.prices[c][Ax]) : [c][Ax-1][c][Ax-1]t =
        let utils : trm.utility [ns][nd] = trm.utility mp p tau
        let tr = trm.age_transition mp
        let ctp = ctp_from_utils mp tr utils ev
    ...
    def ed_ded_price_all_man [n][c][Ax][ns][nd] (mp:trm.mp[n][c][Ax][ns][nd]) (sa_max:i64) (p:trm.prices[c][Ax]) : ([c][Ax-1]t, [c][Ax-1][c][Ax-1]t, [n]i64, [n]i64, [n]i64) =
        let sol_evs (tau:i64) : ([ns]t, [ns][nd]t, i64, i64, i64) =
            let utils : trm.utility [ns][nd] = trm.utility mp p tau
            let tr = trm.age_transition mp
            let ev0 = trm.ev0 mp
    ...
    ------- ed_ded using all manual derivatives instead and SA instead of NK for solving the model
    def ed_ded_price_all_man_sa [n][c][Ax][ns][nd] (mp:trm.mp[n][c][Ax][ns][nd]) (sa_max:i64) (p:trm.prices[c][Ax]) : ([c][Ax-1]t, [c][Ax-1][c][Ax-1]t, [n]i64, [n]i64, [n]i64) =
        let sol_evs (tau:i64) : ([ns]t, [ns][nd]t, i64, i64, i64) =
            let utils : trm.utility [ns][nd] = trm.utility mp p tau
            let tr = trm.age_transition mp
            let ev0 = trm.ev0 mp
